...
const ProfileImgEdit = async (e: any,cookies:any, setCookie:any, removeCookie:any) => {
			...
			console.log("imgae incoding after : ", compressedFile);
                            //인코딩한 이미지를 브라우저에 프리뷰 하기 위해 state 정의
				// setUploadImage(String(compressedFile));                              //인코딩한 이미지를 서버에 전송하기 위한 state 정의
        console.log(String(compressedFile));   
        authAxios.post('/api/user/changeProfileImg', { img: String(compressedFile) }).then((res) => {
